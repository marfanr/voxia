...
impl BitMap {
    ...
    pub fn update_base(&mut self, base: *mut u64) {
        self.base = UnsafeCell::new(base);
    }
    ...
    pub fn setup(&mut self, ctx: *mut MemoryContext) {
        ...
        let bitmap_words = ceil_div(total_block, 64); // exp start
        let bitmap_bytes = align_up(bitmap_words * 8, BLOCK_SIZE);
    ...
    pub unsafe fn alloc_debug(&self, block: usize) {
        let bitmap = self.as_slice();
        serialfmt!("bitmpa len {}\n", bitmap.len() << 6);
        ...
        for bit in 0..self.total_blocks {
            ...
            if bitmap[word_idx] & (1u64 << bit_offset) == 0 {
                ...
                if consecutive == block {
                    ...
                    serialfmt!(
                        "alloc debug ({} block) {:#x} ({}) to {:#x} ({})\n",
                        block,
                        start_bit * BLOCK_SIZE,
                        start_bit / 64,
                        (start_bit + block) * BLOCK_SIZE,
                        start_bit / 64 + block
                    );
