...
/* CONTET OPS*/
int virtio_ctx_bind_resource(struct graphic_context* ctx,
                             struct graphic_resource* res) {
	if (!ctx || !res || !g_virtio_gpu)
		return -1;

	log("Virtio-GPU API", "Binding resource %d to context %d", res->id,
	    ctx->id);

	if (g_virtio_gpu->virtio_gpu_ctx_attach_resource(ctx->id, res->id) !=
	    0) {
		log("Virtio-GPU API",
		    "Failed to bind resource %d to context %d", res->id,
		    ctx->id);
		return -1;
	}

	log("Virtio-GPU API", "Resource %d bound to context %d successfully",
	    res->id, ctx->id);
	return 0;
}

int virtio_ctx_transfer(struct graphic_context* ctx,
                        struct graphic_resource* res,
	...
	if (res->bind & GRAPHIC_BIND_2D) {
		return g_virtio_gpu->virtio_gpu_transfer_to_host_2d(
		    cmd->offset, cmd->resource_id, cmd->x, cmd->y, cmd->w,
		    cmd->h);
	}

	return g_virtio_gpu->virtio_gpu_transfer_to_host_3d(
	    ctx->id, cmd->resource_id, cmd->offset, cmd->level, cmd->x, cmd->y,
	    cmd->z, cmd->w, cmd->h, cmd->d, stride, layer_stride);
}

int virtio_ctx_transfer2(struct graphic_context* ctx,
                         struct graphic_resource* res,
	...
	return g_virtio_gpu->virtio_gpu_transfer_to_host_3d(
	    ctx->id, cmd->resource_id, cmd->offset, cmd->level, cmd->x, cmd->y,
	    cmd->z, cmd->w, cmd->h, cmd->d, cmd->stride, cmd->layer_stride);
}
int virtio_ctx_submit(struct graphic_context* ctx, const void* commands,
                      size_t size) {
	if (!ctx || !commands || !g_virtio_gpu)
		return -1;
	return g_virtio_gpu->virtio_gpu_submit_3d(ctx->id, (void*)commands,
	                                          size);
}

int virtio_ctx_transfer_from(struct graphic_context* ctx,
                             struct graphic_resource* res,
                             struct graphic_transfer_cmd* cmd) {
	if (!ctx || !res || !cmd || !g_virtio_gpu)
		return -1;
	...
	return g_virtio_gpu->virtio_gpu_transfer_from_host_3d(
	    ctx->id, cmd->resource_id, cmd->offset, cmd->level, cmd->x, cmd->y,
	    cmd->z, cmd->w, cmd->h, cmd->d, stride, layer_stride);
}
...
extern "C" int virtio_create_context(struct graphic_device* device,
                                     struct graphic_context_desc* desc,
                                     struct graphic_context** ctx) {
	if (!device || !ctx || !g_virtio_gpu || !desc)
		return -1;

	if (g_virtio_gpu->virtio_gpu_ctx_create(
	        desc->id, desc->nlen, desc->context_init, desc->name) != 0) {
		return -2;
	}
	log("Virtio-GPU API", "created context with id %d\n", desc->id);
	*ctx = (struct graphic_context*)kalloc(sizeof(struct graphic_context) +
	                                       desc->nlen + 1);
	memset(*ctx, 0, sizeof(struct graphic_context) + desc->nlen + 1);

	(*ctx)->name =
	    (char*)((uintptr_t)*ctx + sizeof(struct graphic_context));
...
extern "C" int virtio_create_resource(struct graphic_device* device,
                                      struct graphic_resource_desc* desc,
                                      struct graphic_resource** res) {
	if (!device || !res || !g_virtio_gpu)
		return -1;

	log("Virtio-GPU API", "Creating graphic resource for device %d: %dx%d",
	    device->id, desc->width, desc->height);
	...
	if (device->is_3d && !(desc->bind & GRAPHIC_BIND_2D)) {
		...
		case GRAPHIC_RESOURCE_BUFFER:
			...
			break;
		default:
			log("Virtio-GPU API", "Unsupported resource type: %d",
			    desc->type);
			return -1;
		}
		...
		if (desc->ctx == NULL) {
			log("Virtio-GPU API",
			    "Failed to create Resource %d: Context is NULL",
			    desc->id);
			return -1;
		}
		...
		if (g_virtio_gpu->virtio_gpu_resource_create_3d(
		        desc->ctx->id, desc->id, target, format, bind_flags,
		        desc->width, desc->height, desc->depth,
		        desc->array_size, desc->mip_levels, desc->sample_counts,
		        0) == 0) {
			// log("Virtio-GPU API",
		...
		if (g_virtio_gpu->virtio_gpu_create_resource(
		        desc->id, desc->width, desc->height) == 0) {
			// log("Virtio-GPU API",
			//     "Resource %d (2D Cursor) created successfully",
			//     desc->id);
		} else {
			log("Virtio-GPU API",
			    "Failed to create Resource %d (2D Cursor)",
			    desc->id);
			return -1;
		}
	}
	...
	*res =
	    (struct graphic_resource*)kalloc(sizeof(struct graphic_resource));
...
int virtio_destroy_resource(struct graphic_device* device,
                            struct graphic_resource* res) {
	if (!device || !res || !g_virtio_gpu)
		return -1;

	log("Virtio-GPU API",
	    "Destroying graphic resource for device %d: id=%d", device->id,
	    res->id);
	...
	if (res->vaddr && res->paddr && res->size > 0) {
		IOForge::IOUtils::DMAFree((void*)res->paddr, (void*)res->vaddr,
		                         res->size);
	}
	kfree2(res);
...
int virtio_destroy_context(struct graphic_device* device,
                          struct graphic_context* ctx) {
...
int virtio_resource_attach_backing(struct graphic_device* device,
                                   struct graphic_resource* res) {
	...
	int result =
	    g_virtio_gpu->virtio_gpu_attach_backing(res->id, 1, entries2);
	if (result != 0) {
		log("Virtio-GPU API",
		    "Failed to attach backing forresource id=%d", res->id);
		return -1;
	}
...
int virtio_scanout_set(struct graphic_device* device,
                       struct graphic_scanout* scanout,
                       struct graphic_resource* res) {
	if (!g_virtio_gpu || !device || !scanout || !res)
		return -1;
	int ret = g_virtio_gpu->virtio_gpu_set_scanout(
	    scanout->id, res->id, 0, 0, scanout->width, scanout->height);

	if (ret == 0) {
		scanout->res = res;
		int flush_ret = g_virtio_gpu->virtio_gpu_resource_flush(
		    res->id, 0, 0, res->width, res->height);
		...
		if (last_scanout_res_id != res->id) {
			...
			g_virtio_gpu->virtio_gpu_update_cursor(
			    g_virtio_gpu->cursor_res_id_, scanout->id,
			    g_virtio_gpu->cursor_x_, g_virtio_gpu->cursor_y_,
			    g_virtio_gpu->cursor_hot_x_,
			    g_virtio_gpu->cursor_hot_y_);
...
int virtio_resource_flush(struct graphic_device* device,
                          struct graphic_resource* res, uint32_t x, uint32_t y,
                          uint32_t width, uint32_t height) {
	if (!g_virtio_gpu || !device || !res)
		return -1;

	return g_virtio_gpu->virtio_gpu_resource_flush(res->id, x, y, width,
	                                               height);
}

int virtio_update_cursor(struct graphic_device* device,
                         struct graphic_scanout* scanout,
                         struct graphic_resource* res, uint32_t hot_x,
                         uint32_t hot_y) {
	if (!g_virtio_gpu || !device || !scanout || !res)
		return -1;
	return g_virtio_gpu->virtio_gpu_update_cursor(
	    res->id, scanout->id, scanout->x, scanout->y, hot_x, hot_y);
}

int virtio_move_cursor(struct graphic_device* device,
                       struct graphic_scanout* scanout, uint32_t x,
                       uint32_t y) {
...
int VirtioGpu::virtio_gpu_get_display_info() {
	...
	struct virtio_gpu_ctrl_hdr* test_cmd =
	    (struct virtio_gpu_ctrl_hdr*)IOUtils::DMAAlloc(sizeof(*test_cmd),
	                                                   &test_phys);
	...
	struct virtio_gpu_resp_display_info* test_resp =
	    (struct virtio_gpu_resp_display_info*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_resp_display_info), &test_resp_phys);
	...
	int result =
	    virtio_gpu_send_command((void*)test_phys, sizeof(*test_cmd),
	                            (void*)test_resp_phys, sizeof(*test_resp));
	...
	IOUtils::DMAFree((void*)test_resp_phys, (void*)test_resp,
	                 sizeof(*test_resp));
...
int VirtioGpu::virtio_gpu_unref_resource(uint32_t resource_id) {
	...
	struct virtio_gpu_resource_unref* cmd =
	    (struct virtio_gpu_resource_unref*)IOUtils::DMAAlloc(sizeof(*cmd),
	                                                         &cmd_phys);
...
int VirtioGpu::virtio_gpu_attach_backing(uint32_t resource_id,
                                         uint32_t nr_entries,
                                         struct virtio_gpu_mem_entry* entries) {
	uintptr_t cmd_phys = 0;
	size_t cmd_size = sizeof(struct virtio_gpu_resource_attach_backing) +
	                  sizeof(struct virtio_gpu_mem_entry) * nr_entries;
	struct virtio_gpu_resource_attach_backing* cmd =
	    (struct virtio_gpu_resource_attach_backing*)IOUtils::DMAAlloc(
	        cmd_size, &cmd_phys);
	...
	if (virtio_gpu_send_command((void*)cmd_phys, cmd_size, (void*)resp_phys,
	                            sizeof(*resp)) < 0) {
		log(mod, "Failed to create resource");
		IOUtils::DMAFree((void*)cmd_phys, (void*)cmd, cmd_size);
		IOUtils::DMAFree((void*)resp_phys, (void*)resp, sizeof(*resp));
...
int VirtioGpu::virtio_gpu_set_scanout(uint32_t scanout_id, uint32_t resource_id,
                                      uint32_t x, uint32_t y, uint32_t width,
                                      uint32_t height) {
	uintptr_t cmd_phys = 0;
	struct virtio_gpu_set_scanout* cmd =
	    (struct virtio_gpu_set_scanout*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_set_scanout), &cmd_phys);
	...
	if (virtio_gpu_send_command((void*)cmd_phys,
	                            sizeof(struct virtio_gpu_set_scanout),
	                            (void*)resp_phys, sizeof(*resp)) < 0) {
		log(mod, "Failed to create resource");
		IOUtils::DMAFree((void*)cmd_phys, (void*)cmd,
		                 sizeof(struct virtio_gpu_set_scanout));
		IOUtils::DMAFree((void*)resp_phys, (void*)resp, sizeof(*resp));
		return -1;
	}
...
int VirtioGpu::virtio_gpu_transfer_to_host_2d(uint64_t offset,
                                              uint32_t resource_id, uint32_t x,
                                              uint32_t y, uint32_t width,
                                              uint32_t height) {
	uintptr_t cmd_phys = 0;
	struct virtio_gpu_transfer_to_host_2d* cmd =
	    (struct virtio_gpu_transfer_to_host_2d*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_transfer_to_host_2d), &cmd_phys);
	...
	if (virtio_gpu_send_command(
	        (void*)cmd_phys, sizeof(struct virtio_gpu_transfer_to_host_2d),
	        (void*)resp_phys, sizeof(*resp)) < 0) {
		log(mod, "Failed to create resource");
		IOUtils::DMAFree((void*)cmd_phys, (void*)cmd,
...
int VirtioGpu::virtio_gpu_resource_flush(uint32_t resource_id, uint32_t x,
                                         uint32_t y, uint32_t width,
                                         uint32_t height) {

	uintptr_t cmd_phys = 0;
	struct virtio_gpu_resource_flush* cmd =
	    (struct virtio_gpu_resource_flush*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_resource_flush), &cmd_phys);
...
int VirtioGpu::virtio_gpu_ctx_create(uint32_t ctx_id, uint32_t nlen,
                                     uint32_t context_init, char* debug_name) {
	uintptr_t cmd_phys = 0;
	struct virtio_gpu_ctx_create* cmd =
	    (struct virtio_gpu_ctx_create*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_ctx_create), &cmd_phys);
	...
	if (virtio_gpu_send_command((void*)cmd_phys,
	                            sizeof(struct virtio_gpu_ctx_create),
	                            (void*)resp_phys, sizeof(*resp)) < 0) {
		log(mod, "Failed to create resource");
		IOUtils::DMAFree((void*)cmd_phys, (void*)cmd,
		                 sizeof(struct virtio_gpu_ctx_create));
...
int VirtioGpu::virtio_gpu_ctx_destroy(uint32_t ctx_id) {
	...
	struct virtio_gpu_ctx_destroy* cmd =
	    (struct virtio_gpu_ctx_destroy*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_ctx_destroy), &cmd_phys);
	...
	if (virtio_gpu_send_command((void*)cmd_phys,
	                            sizeof(struct virtio_gpu_ctx_destroy),
	                            (void*)resp_phys, sizeof(*resp)) < 0) {
...
int VirtioGpu::virtio_gpu_resource_create_3d(
    uint32_t ctx_id, uint32_t resource_id, uint32_t target, uint32_t format,
    uint32_t bind, uint32_t width, uint32_t height, uint32_t depth,
    uint32_t array_size, uint32_t last_level, uint32_t nr_samples,
    uint32_t flags) {

	uintptr_t cmd_phys = 0;
	struct virtio_gpu_resource_create_3d* cmd =
	    (struct virtio_gpu_resource_create_3d*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_resource_create_3d), &cmd_phys);
	...
	if (virtio_gpu_send_command(
	        (void*)cmd_phys, sizeof(struct virtio_gpu_resource_create_3d),
	        (void*)resp_phys, sizeof(*resp)) < 0) {
		log(mod, "Failed to create resource");
		IOUtils::DMAFree((void*)cmd_phys, (void*)cmd,
...
int VirtioGpu::virtio_gpu_ctx_attach_resource(uint32_t ctx_id,
                                              uint32_t resource_id) {
	uintptr_t cmd_phys = 0;
	struct virtio_gpu_ctx_resource* cmd =
	    (struct virtio_gpu_ctx_resource*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_ctx_resource), &cmd_phys);
	...
	if (virtio_gpu_send_command((void*)cmd_phys,
	                            sizeof(struct virtio_gpu_ctx_resource),
	                            (void*)resp_phys, sizeof(*resp)) < 0) {
...
int VirtioGpu::virtio_gpu_submit_3d(uint32_t ctx_id, void* cmd_buf,
                                    uint32_t cmd_buf_size) {
	uintptr_t cmd_phys = 0;
	size_t total_size = sizeof(struct virtio_gpu_cmd_submit) + cmd_buf_size;
	struct virtio_gpu_cmd_submit* cmd =
	    (struct virtio_gpu_cmd_submit*)IOUtils::DMAAlloc(total_size,
	                                                     &cmd_phys);
	...
	memcopy((char*)cmd + sizeof(struct virtio_gpu_cmd_submit), cmd_buf,
	        cmd_buf_size);
	...
	if (virtio_gpu_send_command((void*)cmd_phys, total_size,
	                            (void*)resp_phys, sizeof(*resp)) < 0) {
...
int VirtioGpu::virtio_gpu_transfer_to_host_3d(
    uint32_t ctx_id, uint32_t resource_id, uint64_t offset, uint32_t level,
    uint32_t x, uint32_t y, uint32_t z, uint32_t w, uint32_t h, uint32_t d,
    uint32_t stride, uint32_t layer_stride) {
	uintptr_t cmd_phys = 0;
	struct virtio_gpu_transfer_host_3d* cmd =
	    (struct virtio_gpu_transfer_host_3d*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_transfer_host_3d), &cmd_phys);
...
int VirtioGpu::virtio_gpu_transfer_from_host_3d(
    uint32_t ctx_id, uint32_t resource_id, uint64_t offset, uint32_t level,
    uint32_t x, uint32_t y, uint32_t z, uint32_t w, uint32_t h, uint32_t d,
    uint32_t stride, uint32_t layer_stride) {
	uintptr_t cmd_phys = 0;
...
int VirtioGpu::virtio_gpu_resource_create_blob(
	...
	struct virtio_gpu_resource_create_blob* cmd =
	    (struct virtio_gpu_resource_create_blob*)IOUtils::DMAAlloc(
	        sizeof(struct virtio_gpu_resource_create_blob), &cmd_phys);
	...
	if (virtio_gpu_send_command(
	        (void*)cmd_phys, sizeof(struct virtio_gpu_resource_create_blob),
	        (void*)resp_phys, sizeof(*resp)) < 0) {
		log(mod, "Failed to send RESOURCE_CREATE_BLOB command");
		IOUtils::DMAFree(
		    (void*)cmd_phys, (void*)cmd,
