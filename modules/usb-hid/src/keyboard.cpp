...
void HIDKeyboard::load(ioforge_usb_device* dev) {
	...
	serial2_printf("pipe at 0x%lx\n", pipe);

	uint8_t b = dev_->endpoints[0].interval;
	uint16_t interval_ms = b; // For Low/Full speed, interval is exactly b ms
	if (interval_ms < 1)
		interval_ms = 1;
...
// the shift state, but to keep your "table" request, we'll map both to the same
// KEY_ enum.
...
void HIDKeyboard::parse_report(const uint8_t* data, size_t len) {
	...
	for (int i = 0; i < 6; i++) {
		...
		if (!still_held) {
			...
			if (code != KEY_NONE)
				input_report_key(
				    &instance->dev_->controller->service, code,
				    0);
		}
	}
