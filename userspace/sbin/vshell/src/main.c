...
int main(void) {
	...
	while (1) {
		printf("\033[1;32m%s@voxia\033[0m:\033[1;34m%s\033[0m# ", "root", "/");
		fflush(stdout);
		linelen = 0;
		...
		if (pid == 0) {
			...
			// hardcoded envp
			char* envp[] = {"TERMINFO=/usr/share/terminfo",
			                "TERM=xterm", "HOME=/root", NULL};

			int res = execve(linebuf, argv, envp);
			if (res < 0) {
			...
			if (WIFEXITED(status)) {
				...
				if (exit_code != 1 ) {
					printf("\033[38;5;205m[Process exited "
					       "with status %d]\033[0m\n",
					       exit_code);
				}
			} else if (WIFSIGNALED(status)) {
				printf("\033[38;5;205m[Process terminated "
				       "by signal %d]\033[0m\n",
				       WTERMSIG(status));
			}
		}
