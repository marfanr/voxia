...
uint64_t
setup_user_stack(uint64_t argc, uint64_t vaddr, uint64_t *rsp, char **argv, char **envp)
{
...
void
setup_stack(uintptr_t stack_addr)
{
...
uint64_t
pflags_to_page_flags(uint32_t pflags)
{
    uint64_t flags = 0;
    if (pflags & PROT_READ)
        flags |= PAGE_PRESENT;
...
int
spawn(const char *path, char *argv[], char *envp[])
{
    serial_trace("\nspawn %s\n", path);
    ...
    uint8_t *file = (uint8_t *)(phys_base_alloc(2 + stats.size / 4096));
    int      read = vfs_read(fd, file, stats.size);
    if (read != 0)
    ...
    uint8_t    *str_tab = (uint8_t *)((uint64_t)ehdr + shdr[ehdr->e_shstrndx].sh_offset);
    ...
    for (uint64_t i = 0; i < ehdr->e_phnum; i++)
    {
        if (phdr2->p_type == PT_DYNAMIC)
        {
            ...
            dyn = (Elf64_Dyn *)((uint8_t *)(file + phdr2->p_offset));
        }
        else if (phdr2->p_type == PT_LOAD)
        {
            void     *a               = (void *)(phys_base_alloc(1 + phdr2->p_memsz / 4096));
            uintptr_t allignment_vadr = phdr2->p_vaddr & ~0xFFF;
            uintptr_t offset_page     = phdr2->p_vaddr - allignment_vadr;

            // if ((uint64_t)a % 0x1000 != 0) {
            //     serial_trace("ERROR: PADDRNOT ALIGNMENT AT 4KB\n");
            // }
            // if (phdr2->p_vaddr % 0x1000 != 0) {
            //     serial_trace("ERROR: VADDR NOT ALIGNMENT AT 4KB\n");
            //     serial_trace("alignment vaddr : 0x%x\n", allignment_vadr);
            // }
            uint64_t round_up_size = (phdr->p_memsz + 0xFFF) & ~0xFFF;

            serial_trace("mapping 0x%x to 0x%x size %d until 0x%x (real end 0x%x)\n",
                         allignment_vadr, a, phdr2->p_memsz, allignment_vadr + phdr2->p_memsz,
            ...
            paging_mmap_fill(page_, phdr2->p_vaddr, VIRT2PHYS((uintptr_t)a), round_up_size / 4096,
                             0b111);

            paging_reload((page_t)VIRT2PHYS((uint64_t)page_));
            serial_trace("mapped addr : 0x%x\n",
                         vaddr_to_paddr((page_t)((uint64_t)page_), phdr2->p_vaddr));
        }
    ...
    if (dyn != 0)
    {
        ...
        for (int i = 0; dyn[i].d_tag != DT_NULL; i++)
        {
            ...
            {
                case DT_STRTAB:
                    strtab = (uint8_t *)((uint64_t)file + dyn[i].d_un.d_ptr - base);
                ...
                case DT_PLTRELSZ:
                    plt_relz = dyn[i].d_un.d_val / sizeof(Elf64_Rela);
                    serial_trace("relsz : %d\n", plt_relz);
                    break;

                case DT_PLTGOT:
                    serial_trace("pltgot : 0x%x\n", dyn[i].d_un.d_ptr);
                    void *c = (void *)(phys_base_alloc(1 + plt_relz * sizeof(Elf64_Rela) / 4096));
                    uintptr_t allignment_vadr = dyn[i].d_un.d_ptr & ~0xFFF;
                    ...
                    paging_mmap(page_, allignment_vadr, VIRT2PHYS((uint64_t)c), 0b111);
                    // paging_reload(VIRT2PHYS(page_));
                    break;

                case DT_JMPREL:
                    serial_trace("jmprel : 0x%x\n", dyn[i].d_un.d_ptr);
                    plt_rela = (Elf64_Rela *)(file + (dyn[i].d_un.d_ptr - base));
                    break;

                case DT_SYMTAB:
                    serial_trace("symtab : 0x%x\n", dyn[i].d_un.d_ptr);
                    symbols = (Elf64_Sym *)((uint64_t)file + (dyn[i].d_un.d_ptr - base));
                    break;
        ...
        if (strtab && dyn_needed)
        {
            ...
            const char *library_name = (const char *)(strtab + dyn_needed);
            ...
            uint8_t *lib_strtab = (uint8_t *)((uint64_t)ehdr + shdr[ehdr->e_shstrndx].sh_offset);

            // parse secton header pada library
            for (uint64_t i = 0; i < ehdr->e_shnum; i++)
            {
                char *name = (char *)(lib_strtab + shdr->sh_name);
                if (strncmp(name, ".dynsym", 7) == 0)
                {
                    serial_trace("dynamic symbol found\n");
                    serial_trace("size : %d\n", shdr->sh_size);
                    lib_dynsym      = (Elf64_Sym *)(lib->entry + shdr->sh_offset);
                    lib_dynsym_size = shdr->sh_size;
                }
                else if (strncmp(name, ".dynstr", 7) == 0)
                {
                    serial_trace("dynamic string found\n");
                    dynstr = (uint8_t *)(lib->entry + shdr->sh_offset);
                }
            ...
            if (lib_dynsym != 0)
            {
                for (int i = 0; i < lib_dynsym_size / sizeof(Elf64_Sym); i++)
                {
                    serial_trace("Function %s: Start: 0x%x, "
                                 "Size: %d bytes, end : 0x%x\n",
                                 dynstr + lib_dynsym[i].st_name, lib_dynsym[i].st_value,
                                 lib_dynsym[i].st_size,
                                 lib_dynsym[i].st_value + lib_dynsym[i].st_size);
                }
            }
            ...
            for (uint64_t i = 0; i < ehdr->e_phnum; i++)
            {
                ...
                {
                    serial_trace("dynamic pheader found\n");
                    libdyn = (Elf64_Dyn *)((uint8_t *)(lib->entry + phdr->p_offset));
                }
                else if (phdr->p_type == PT_LOAD)
                {
                    void     *a               = (void *)(phys_base_alloc(1 + phdr->p_memsz / 4096));
                    uintptr_t allignment_vadr = phdr->p_vaddr & ~0xFFF;
                    uintptr_t offset_page     = phdr->p_vaddr - allignment_vadr;
                    ...
                    uint64_t round_up_size = (phdr->p_memsz + 0xFFF) & ~0xFFF;
                    paging_mmap_fill(page_, allignment_vadr, VIRT2PHYS((uint64_t)a),
                                     (round_up_size / 4096), 0b111);
                    // }
                    serial_trace("mapping 0x%x to "
                                 "0x%x\n",
                                 allignment_vadr, VIRT2PHYS((uint64_t)a) + offset_page);
                    // paging_reload(VIRT2PHYS((uint64_t)page_));
                    // serial_trace("mapped addr : 0x%x\n",
                    // vaddr_to_paddr(((uint64_t)page_), phdr->p_vaddr));

                    for (uint64_t j = 0; j < phdr->p_memsz; j++)
                    {
                        *(uint8_t *)((uint64_t)a + offset_page + j) =
                            *(uint8_t *)((uint64_t)lib->entry + phdr->p_offset + j);
                    }
                }
            ...
            if (libdyn != 0)
            {
                for (int i = 0; libdyn[i].d_tag != DT_NULL; i++)
                {
                    switch (libdyn[i].d_tag)
                    {
                        case DT_INIT:
                            // case DT_INIT_ARRAY:
                            serial_trace("dt init : 0x%x\n", libdyn[i].d_un.d_ptr);
                            init_func_addr = (uint8_t *)(libdyn[i].d_un.d_ptr);
                            // void (*init) ()
                            //     = (void (*) ())lib->entry
                            //       + libdyn[i].d_un.d_ptr;
                            // init ();
        ...
        if (plt_rela != 0)
        {
            for (int i = 0; i < plt_relz; i++)
            {
                ...
                Elf64_Sym *sym      = &symbols[Elf64_R_SYM(plt_rela[i].r_info)];
                ...
                if (lib_dynsym != 0)
                {
                    for (int k = 0; k < lib_dynsym_size / sizeof(Elf64_Sym); k++)
                    {
                        const char *lib_dynsym_name =
                            (const char *)(dynstr + lib_dynsym[k].st_name);
                        if (strncmp(lib_dynsym_name, sym_name, strlen(sym_name)) == 0)
                        {
                            serial_trace("found symbol %s\n", lib_dynsym_name);
                            serial_trace("symbol value : "
                                         "0x%x\n",
                                         lib_dynsym[k].st_value);
                            serial_trace("relocation value : "
                                         "0x%x\n",
                                         lib_dynsym[k].st_value + plt_rela[i].r_addend);
                            serial_trace(
                                "mapped addr : 0x%x\n",
                                vaddr_to_paddr((page_t)((uint64_t)page_),
                                               lib_dynsym[k].st_value + plt_rela[i].r_addend));

                            serial_trace("ok 0x%x\n", *(uint64_t *)(plt_rela[i].r_offset));
                            *(uint64_t *)(plt_rela[i].r_offset) =
                                (uint64_t)(lib_dynsym[k].st_value + plt_rela[i].r_addend);
    ...
    uintptr_t s = (uintptr_t)(phys_base_alloc(2049));
    ...
    // mapping to 2GB
    paging_mmap_fill(page_, 256 * GB, VIRT2PHYS((uint64_t)s), 2049, 0b111);
    ...
    aligned_stack           = setup_user_stack(argc, aligned_stack, stack_top, argv, envp);
    ...
    task_add(path, entry, TASK_READY, TASK_PRIORITY_MEDIUM, (page_t)VIRT2PHYS(page_), aligned_stack,
             param);
