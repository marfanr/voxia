...
int spawn(const char* path, char* argv[], char* envp[]) {
	...
	for (uint64_t i = 0; i < ehdr->e_phnum; i++) {
		if (phdr2->p_type == PT_DYNAMIC) {
			...
			void* a =
			    (void*)(vxPhysBaseAlloc(1 + phdr2->p_memsz / 4096));
	...
	if (dyn != 0) {
		...
		for (int i = 0; dyn[i].d_tag != DT_NULL; i++) {
			...
			case DT_PLTGOT:
				...
				void* c = (void*)(vxPhysBaseAlloc(
				    1 + plt_relz * sizeof(Elf64_Rela) / 4096));
		...
		if (plt_rela != 0) {
			for (int i = 0; i < plt_relz; i++) {
				...
				Elf64_Sym* sym =
				    &symbols[Elf64_R_SYM(plt_rela[i].r_info)];
