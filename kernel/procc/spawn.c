  ...
  for (uint64_t i = 0; i < ehdr->e_phnum; i++) {
    if (phdr2->p_type == PT_DYNAMIC) {
      ...
      serial_trace("mapping 0x%x to 0x%x size %d until 0x%x (real end 0x%x)\n",
                   ...
                   allignment_vadr + phdr2->p_memsz, allignment_vadr + 4096);
      ...
      paging_mmap_fill(page_, phdr2->p_vaddr, VIRT2PHYS((uintptr_t)a),
  ...
  if (dyn != 0) {
    ...
    for (int i = 0; dyn[i].d_tag != DT_NULL; i++) {
      ...
      case DT_PLTGOT:
        ...
        void *c =
            (void *)(phys_base_alloc(1 + plt_relz * sizeof(Elf64_Rela) / 4096));
