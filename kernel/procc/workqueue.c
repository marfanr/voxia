...
workqueue_process()
{
    ...
    LOG_INFO("workqueue", "worker thread running on core %d", core->core_id);
    while (true)
    {
        ...
        for (uint16_t i = 0; i < VOXIA_MAX_WORKQUEUE_EACH_CORE; i++)
        {
            ...
            // LOG_DEBUG("WORKQUEUE", "checking workqueue task in slot %d", i);
            if (workqueue->dependency)
            {
                boolean_t dependency_done = true;
                // LOG_DEBUG("WORKQUEUE", "workqueue task in slot %d has dependency size %d", i,
                //           workqueue->dependency->size);

                for (size_t j = 0; j < workqueue->dependency->size; j++)
                {
                    auto dep = workqueue->dependency->data[j];
                    if (!dep)
                    {
                        LOG_ERROR("WORKQUEUE", "dependency %d is null", j);
                        return;
                    }

                    // LOG_DEBUG("WORKQUEUE", "checking dependency %d", j);
                    if (dep->in_use)
                    {
            ...
            // LOG_INFO("workqueue", "core %d executing task in slot %d", core->core_id, i);
...
workqueue_t *
vxAddWorkqueueTask(void (*task)(void *), void *arg, vector(workqueue_ptr_t) * dependency)
{
    // find core with lower workqueue count
    each_core_data *core = vxGetCoreDataByCoreID(1);
    LOG_INFO("workqueue", "adding task to core %d", core->core_id);
    spin_acquire(&lock);
    ...
    for (uint16_t i = 0; i < VOXIA_MAX_WORKQUEUE_EACH_CORE; i++)
    {
        if (!core->workqueue[i].in_use)
        {
            ...
            core->workqueue_count += 1;
            LOG_INFO("workqueue", "task added to core %d slot %d", core->core_id, i);
            spin_release(&lock);
            return &core->workqueue[i];
        }
    }
    LOG_ERROR("workqueue", "no available slot in workqueue on core %d", core->core_id);
...
INIT(Workqueue)
{
    for (uint16_t i = 1; i < vxGetNumberOfCores(); i++)
