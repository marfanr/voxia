...
static long pty_master_write(vnode_t* vnode, void* buf, size_t len, size_t offset) {
    ...
    if (!fd) return -EINVAL;
    auto internal = (struct internal_pty*)fd->private_data;
    if (!internal || internal->slave_closed) return -EPIPE;
...
static int pty_master_read(vnode_t* vnode, void* buf, size_t len, size_t offset) {
    ...
    while (1) {
        ...
        if (internal->slave_closed) return 0;
...
static int pty_master_poll(vnode_t* vnode, thread_t* waiter) {
    ...
    if ((head - tail) == 0 && waiter) internal->master_waiter = waiter;
...
static int pty_master_close(vnode_t* vnode) {
    auto fd = (struct file_descriptor*)vnode->vnode_private;
    if (!fd) return -EINVAL;
    auto internal = (struct internal_pty*)fd->private_data;
    if (internal) {
...
static int pty_slave_ioctl(vnode_ptr_t vnode, uint32_t req, void* arg) {
    ...
    case TIOCGWINSZ: {
        /* Mengembalikan ukuran yang telah diset (tidak hardcode lagi) */
        memcopy(arg, &internal->ws, sizeof(struct win_size));
        return 0;
    }
    case TIOCSWINSZ: {
        /* Menyimpan ukuran dinamis yang dikirim compositor / vxterm */
        memcopy(&internal->ws, arg, sizeof(struct win_size));
        /* TODO (Future): Kirim SIGWINCH ke internal->foreground jika ukurannya berubah */
        return 0;
    }
    case TIOCGPGRP: {
...
static int pty_slave_read(vnode_t* vnode, void* buf, size_t len, size_t offset) {
    ...
    while (1) {
        ...
        /* EOF jika master sudah ditutup (menghindari thread hang selamanya) */
        if (internal->master_closed) return 0;

        /* Catatan: Untuk implementasi O_NONBLOCK di sisi slave, kamu perlu meneruskan 
           pointer struct file_descriptor (fd) ke vnode->vnode_private seperti pada master,
           bukan langsung struct internal_pty, agar flag O_NONBLOCK dapat dicek di sini. */
...
static int pty_slave_poll(vnode_t* vnode, thread_t* waiter) {
    ...
    if ((head - tail) == 0 && waiter) internal->slave_waiter = waiter;
    return (head - tail) > 0 || internal->master_closed;
}

__attribute__((unused))
static int pty_slave_close(vnode_t* vnode) {
    auto internal = (struct internal_pty*)vnode->vnode_private;
    if (internal) {
...
vops_file_t pty_slave_ops = {
    ...
    .read = pty_slave_read,
    .poll = pty_slave_poll,
    /* .close = pty_slave_close, <-- Uncomment jika struct vops_file_t memiliki fungsi close/release */
};
