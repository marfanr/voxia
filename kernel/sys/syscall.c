...
int
sys_read(int descriptor, char *buffer, uint64_t length)
{
...
uint64_t
sys_open(const char *path, uint64_t flags)
{
...
uint64_t
sys_mmap(uint64_t addr, uint64_t length, uint64_t prot, uint64_t flags, uint64_t fd,
         uint64_t offset)
{
    return 0;
}
