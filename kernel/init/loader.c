...
stivale2_get_tag(struct stivale2_struct* stivale2_struct, uint64_t id) {
	struct stivale2_tag* current_tag =
	    (struct stivale2_tag*)(uintptr_t)
	        stivale2_struct->tags; // uint64_t → uintptr_t → ptr
	...
	for (;;) {
		...
		if (current_tag->identifier == id)
			return current_tag;
		current_tag =
		    (struct stivale2_tag*)(uintptr_t)current_tag->next;
	}
}
...
void build_context_from_stivale2(struct stivale2_struct* stivale2_struct,
	...
	// initialize memori allocator
	{
		...
		if (!memmap_info) {
			serial_trace("FATAL: no memmap tag\n");
			for (;;)
				__asm__ volatile("hlt");
		}

		uint64_t entries = memmap_info->entries;
		...
		for (uint64_t i = 0; i < entries; i++) {
			volatile struct stivale2_mmap_entry* entry =
