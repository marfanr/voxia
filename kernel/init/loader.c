...
static struct stivale2_header_tag_framebuffer framebuffer_hdr_tag = {
    .tag = {.identifier = STIVALE2_HEADER_TAG_FRAMEBUFFER_ID, .next = (uint64_t)&smp_hdr_tag},
    .framebuffer_width  = 0,
...
int
stivale2_mem_entry_type_converter(uint32_t type)
{
    switch (type)
    {
...
{
    {
        ...
        for (uint64_t i = 0; i < modules_info->module_count; i++)
        {
            ...
            if (strncmp(module->string, "boot:///initrd.tar", 18) == 0)
            {
                ...
                LOG_INFO("INIT", "initrd size : %d", module->end - module->begin);
                ctx->initrd_module.size  = module->end - module->begin;
                ctx->initrd_module.start = module->begin;
                break;
            }
        }
        LOG_INFO("INIT", "initrd module found at 0x%x", ctx->initrd_module.start);
    }
    ...
    // initialize memori allocator
    {
        ...
        {
            struct stivale2_mmap_entry *entry = &memmap_info->memmap[i];
            ctx->memory.memory_map[i].base    = entry->base;
            ctx->memory.memory_map[i].length  = entry->length;
            ctx->memory.memory_map[i].type    = stivale2_mem_entry_type_converter(entry->type);
        }
    }

    {
        struct stivale2_struct_tag_rsdp *rsdp_info =
            (struct stivale2_struct_tag_rsdp *)stivale2_get_tag(stivale2_struct,
    ...
    // setup framebuffer
    {
        ...
        ctx->framebuffer.framebuffer_addr   = framebuffer_info->framebuffer_addr;
        ctx->framebuffer.framebuffer_width  = framebuffer_info->framebuffer_width;
        ctx->framebuffer.framebuffer_height = framebuffer_info->framebuffer_height;
        ctx->framebuffer.framebuffer_bpp    = framebuffer_info->framebuffer_bpp;
        ctx->framebuffer.framebuffer_pitch  = framebuffer_info->framebuffer_pitch;

        ctx->framebuffer.blue_mask_shift  = framebuffer_info->blue_mask_shift;
        ctx->framebuffer.blue_mask_size   = framebuffer_info->blue_mask_size;
        ctx->framebuffer.green_mask_shift = framebuffer_info->green_mask_shift;
        ctx->framebuffer.green_mask_size  = framebuffer_info->green_mask_size;
        ctx->framebuffer.red_mask_shift   = framebuffer_info->red_mask_shift;
        ctx->framebuffer.red_mask_size    = framebuffer_info->red_mask_size;
    }
    ...
    // kernel
    {
        ...
        LOG_INFO("KERNEL", "kernel file addr 0x%x ", kernel_info->kernel_file);
        LOG_INFO("KERNEL", "kernel file size %d Kb (%d Mb)", kernel_info->kernel_size / 1024,
