...
__attribute__((section(".limine_requests_start"),
               used)) static volatile uint64_t requests_start[] =
    LIMINE_REQUESTS_START_MARKER;

/* Limine Requests */
__attribute__((section(".limine_requests"),
               used)) static volatile uint64_t base_revision[] =
    LIMINE_BASE_REVISION(6);

__attribute__((section(".limine_requests"),
               used)) static volatile struct limine_framebuffer_request
...
__attribute__((
    section(".limine_requests"),
    used)) static volatile struct limine_rsdp_request rsdp_request = {
...
__attribute__((section(".limine_requests"),
               used)) static volatile struct limine_executable_address_request
    kernel_address_request = {.id = LIMINE_EXECUTABLE_ADDRESS_REQUEST_ID,
                              .revision = 0};

__attribute__((section(".limine_requests"),
               used)) static volatile struct limine_executable_file_request
    kernel_file_request = {.id = LIMINE_EXECUTABLE_FILE_REQUEST_ID,
                           .revision = 0};

__attribute__((section(".limine_requests"),
               used)) static volatile struct limine_executable_cmdline_request
    kernel_cmdline_request = {.id = LIMINE_EXECUTABLE_CMDLINE_REQUEST_ID,
                              .revision = 0};

__attribute__((section(".limine_requests_end"),
               used)) static volatile uint64_t requests_end[] =
...
void build_context_from_limine(init_context_t* ctx) {
	...
	if (kernel_file_request.response != NULL &&
		...
		if (ctx->kernel_raw_addr == 0) {
			ctx->kernel_raw_addr =
			    VIRT2PHYS((uintptr_t)kernel_file->address);
		}
		...
		for (uint64_t i = 0; i < response->module_count; i++) {
			...
			if (module != NULL && module->path != NULL) {
				serial2_printf("found module %s\n",
				               module->path);
				if (strncmp(module->path, "/initrd.tar", 18) ==
				    0) {
					ctx->initrd_module.size = module->size;
					ctx->initrd_module.start =
					    (uintptr_t)module->address;
					serial_trace(
					    "loader: initrd found at %p, size "
					    "%lu\n",
					    (void*)ctx->initrd_module.start,
					    (unsigned long)module->size);
					break;
				}
			}
		}
	...
	if (memmap_request.response != NULL) {
		...
		for (uint64_t i = 0; i < entries; i++) {
			struct limine_memmap_entry* entry =
			    response->entries[i];
			if (entry != NULL) {
				serial_trace("loader: memmap entry %x: "
				             "base=%x, length=%x, type=%x\n",
				             (unsigned long)i,
				             (void*)entry->base,
				             (unsigned long)entry->length,
				             (unsigned long)entry->type);
				ctx->memory.memory_map[i].base = entry->base;
				ctx->memory.memory_map[i].length =
				    entry->length;
				ctx->memory.memory_map[i].type =
				    limine_mem_entry_type_converter(
				        entry->type);
			}
		}
	} else {
