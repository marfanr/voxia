...
static void virtio_scan_capabilities(struct ioforge_pci_device* pci,
                                     uint16_t virtio_cap,
	...
	LOG2_INFO("VIRTIO",
	          ...
	          "offset 0x%lx",
	          cap_type, cap_len, bar, (uint64_t)offset);
	...
	case VIRTIO_PCI_CAP_COMMON_CFG: {
		...
		LOG2_INFO("VIRTIO",
		          "  COMMON_CFG: bar=%d, offset=0x%lx, len=%d", bar,
		          (uint64_t)offset, multiplier);

		break;
	}
	case VIRTIO_PCI_CAP_NOTIFY_CFG: {
		uint32_t multiplier =
		    pci_read32(pci_bus, pci_dev, pci_func, virtio_cap + 16);
		...
		LOG2_INFO("VIRTIO",
		          "  NOTIFY_CFG: bar=%d, offset=0x%lx, multiplier=%d",
		          bar, (uint64_t)offset, multiplier);
		break;
	}
	case VIRTIO_PCI_CAP_ISR_CFG: {
		...
		LOG2_INFO("VIRTIO", "  ISR_CFG: bar=%d, offset=0x%lx", bar,
		          (uint64_t)offset);
		break;
	}
	case VIRTIO_PCI_CAP_DEVICE_CFG: {
		uint32_t multiplier =
		    pci_read32(pci_bus, pci_dev, pci_func, virtio_cap + 12);
		auto virtio_device_cap = &v->device_cfg;
		virtio_device_cap->bar = bar;
		virtio_device_cap->offset = offset;
		virtio_device_cap->length = multiplier;
		virtio_device_cap->cfg_type = VIRTIO_PCI_CAP_DEVICE_CFG;
		virtio_device_cap->cap_next = next_ptr;
		LOG2_INFO("VIRTIO", "  DEVICE_CFG: bar=%d, offset=0x%lx", bar,
		          (uint64_t)offset);
		break;
	}
	}
	serial2_printf("virtio cap: offset 0x%lx\n", v->isr_cfg.offset);
}
		...
		if (pci->base.flags & (uint32_t)IOFORGE_F_VIRTIO) {
			...
			struct ioforge_virtio_device* virtio_device =
			    (struct ioforge_virtio_device*)kalloc(
			        sizeof(struct ioforge_virtio_device));
			memset(virtio_device, 0,
			       sizeof(struct ioforge_virtio_device));
			memcopy(&virtio_device->pci, pci,
			...
			}
			{
				...
				if (isr_bar >= 6 ||
				    virtio_device->pci.bar[isr_bar].iospace) {
					LOG2_ERROR("VIRTIO",
					           "Invalid ISR BAR: %d",
					           isr_bar);
					return;
