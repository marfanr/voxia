...
static void *ehci_alloc(size_t size) {
  ...
  paging_mmap_fill((page_t)PHYS2VIRT((uintptr_t)paging_get_highest_page_map()),
 ...
 */
void ehci_init(struct IoForgePCI *device) {
...
static void ehci_probe_all_ports(int ports, struct ehci_operation *op) {
  ...
  for (uint8_t i = 0; i < ports; i++) {
    ...
    if (available) {
      ...
      usleep(1000);
      config = (struct usb_config_descriptor *)usb_get_descriptor(
      ...
      if (config->iConfiguration > 0) {
        struct usb_string_descriptor *str =
            (struct usb_string_descriptor *)usb_get_descriptor(
                ...
                config->bLength - (sizeof(struct usb_config_descriptor) +
      ...
      if (interface->bNumEndpoints > 0) {
        ...
        while (start < (uint32_t)config + config->wTotalLength) {
          struct usb_endpoint_descriptor *ep =
              (struct usb_endpoint_descriptor *)start;
    ...
    report_retry:
      ...
      if (devClass == 0x3) {
        ...
        setup_packet =
            (struct usb_setup_packet *)ALIGN_UP((uintptr_t)setup_packet, 32);
...
uint8_t *ehci_block_read(void *this, uint64_t offset, size_t _count) {
  // no used in initrd
  return (uint8_t *)((uintptr_t)(((block_device_operations_t *)this)->ctx) +
                     offset);
}

block_device_operations_t *ehci_block_impl() {
  block_device_operations_t *ops = (block_device_operations_t *)phys_base_alloc(
