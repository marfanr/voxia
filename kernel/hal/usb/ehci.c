...
static void  ehci_proccess_async(struct ehci_operation *op, struct ehci_queue_task_descriptor *qtd);
static void  ehci_send_packet_and_receive(struct ehci_operation *op, uint8_t addr,
                                          struct usb_setup_packet *packet, void *data,
                                          uint8_t endpoint);
static void  usb_read_in(struct ehci_operation *op, uint8_t addr, uint32_t length, uint8_t endpoint,
                         void *data);
static void *usb_get_descriptor(struct ehci_operation *op, uint8_t addr, uint8_t type,
...
struct vfs_open_response  *ehci_vfs_open(block_device_operations_t *block_op, const char *path,
                                         int _inode);
 ...
 */
static void *
ehci_alloc(size_t size)
{
    size_t s = 1 + (size / 4096);
    void  *a = (void *)VIRT2PHYS(phys_base_alloc(s));

    paging_mmap_fill((page_t)PHYS2VIRT((uintptr_t)paging_get_highest_page_map()), (uintptr_t)a,
                     (uintptr_t)a, s, 0x3);
    paging_reload(paging_get_highest_page_map());
    paging_add_dma_mapping((uintptr_t)a, (uintptr_t)a, s); // add to mapping data
 ...
 */
void
ehci_init(struct IoForgePCI *device)
{
    serial_trace("EHCI: init\n");

    uintptr_t bar  = device->bar[0].address;
    uintptr_t addr = 0xFFFF8FFF00300000;
    paging_mmap_fill((page_t)PHYS2VIRT((uintptr_t)paging_get_highest_page_map()), addr, bar, 3,
                     0b111);
    paging_reload(paging_get_highest_page_map());
    ...
    inhead  = (struct ehci_queue_head *)ehci_alloc(sizeof(struct ehci_queue_head));
    inhead2 = (struct ehci_queue_head *)ehci_alloc(sizeof(struct ehci_queue_head));
    ...
    inttd =
        (struct ehci_queue_task_descriptor *)ehci_alloc(sizeof(struct ehci_queue_task_descriptor));
 ...
 */
static void
ehci_stop_engine(struct ehci_operation *op)
{
 ...
 */
static void
ehci_reset_engine(struct ehci_operation *op)
{
 ...
 */
static void
ehci_start_engine(struct ehci_operation *op)
{
 ...
 */
static void
ehci_init_que_head(struct ehci_operation *op)
{
 ...
 */
static void
ehci_enable_periodic_schedule(struct ehci_operation *op)
{
 ...
 */
static void
ehci_disable_periodic_schedule(struct ehci_operation *op)
{
    op->usbcmd &= ~EHCI_PERIODIC_SCHEDULE_ENABLE;
}
 ...
 */
static void
ehci_port_reset(struct ehci_operation *op, int port)
{
 ...
 */
static void
ehci_proccess_async(struct ehci_operation *op, struct ehci_queue_task_descriptor *qtd)
{
 ...
 */
static void
ehci_set_address(uint8_t address, struct ehci_operation *op)
{
    struct usb_setup_packet *cmd =
        (struct usb_setup_packet *)ehci_alloc(sizeof(struct usb_setup_packet));
...
ehci_send_packet_and_receive(struct ehci_operation *op, uint8_t addr,
                             struct usb_setup_packet *packet, void *data, uint8_t endpoint)
{
 ...
 */
static void *
usb_get_descriptor(struct ehci_operation *op, uint8_t addr, uint8_t type, uint8_t index,
                   uint8_t len)
{
    struct usb_setup_packet *cmd =
        (struct usb_setup_packet *)ehci_alloc(sizeof(struct usb_setup_packet));
...
static void
usb_read_in(struct ehci_operation *op, uint8_t addr, uint32_t length, uint8_t endpoint, void *data)
{
...
static void
ehci_probe_all_ports(int ports, struct ehci_operation *op)
{
    ...
    for (uint8_t i = 0; i < ports; i++)
    {
        ...
        if (available)
        {
            ...
            struct usb_device_descriptor *desc = (struct usb_device_descriptor *)usb_get_descriptor(
                op, addr, 1, 0, sizeof(struct usb_device_descriptor));
            serial_trace(" USB Port %d : Descriptor Length : %d\n", i, desc->bLength);
            serial_trace(" USB Port %d : Descriptor Type : %d\n", i, desc->bDescriptorType);
            ...
            struct usb_config_descriptor *config =
                (struct usb_config_descriptor *)usb_get_descriptor(
                    op, addr, 2, 0, sizeof(struct usb_config_descriptor));

            // //usleep(1000);
            config = (struct usb_config_descriptor *)usb_get_descriptor(op, addr, 2, 0,
                                                                        config->wTotalLength);

            // if (((config->bDescriptorType != 2) || (config->bLength == 0)) && retrc
            // < 4) {
            //     retrc++;
            // usleep(10000);
            //     goto retry_config;
            ...
            serial_trace("USB Port %d : max power : %dmA\n", i, config->bMaxPower * 2);

            struct usb_interface *interface =
                (struct usb_interface *)((uintptr_t)config + config->bLength);
            ...
            if (config->iConfiguration > 0)
            {
                struct usb_string_descriptor *str =
                    (struct usb_string_descriptor *)usb_get_descriptor(
                        ...
                        config->bLength -
                            (sizeof(struct usb_config_descriptor) + sizeof(struct usb_interface) +
                             interface->bNumEndpoints * sizeof(struct usb_endpoint_descriptor)));
            ...
            ehci_send_packet_and_receive(op, addr, set_config, 0, 0);
            ...
            if (interface->bNumEndpoints > 0)
            {
                ...
                while (start < (uint32_t)config + config->wTotalLength)
                {
                    struct usb_endpoint_descriptor *ep = (struct usb_endpoint_descriptor *)start;
                ...
                serial_send_number(endpoint->bEndpointAddress & 0xF, 16);
                serial_trace("\n endpoint type : %d\n",
                             (endpoint->bEndpointAddress & (1 << 7)) >> 7);
            ...
            int max_retry = 0;
        report_retry:
            ehci_send_packet_and_receive(op, addr, setup_packet, it, 0);
            ...
            if (devClass == 0x3)
            {
                ...
                setup_packet = (struct usb_setup_packet *)ALIGN_UP((uintptr_t)setup_packet, 32);

                setup_packet->bmRequestType = 0b00100001;
                setup_packet->bRequest      = 0x0B;
                setup_packet->wIndex        = interface->bInterfaceNumber;
                setup_packet->wValue        = 1;
                setup_packet->wLength       = 0x0;
                ehci_send_packet_and_receive(op, addr, setup_packet, 0, 0);
...
char
scancode_to_char(uint8_t scancode, uint8_t modifiers)
{
    ...
    if (modifiers)
        return lookup_table[scancode] - 32; // Ubah ke huruf besar jika shift ditekan
...
ioforge_usb_ehci_interrupt()
{
    ...
    if (is_periodic_transaction_is_running)
    {
        ...
        for (int i = 2; i < 8; i++)
        {
            ...
            if (in_data[i] == 0)
                break;
            memcopy((void *)((uintptr_t)vfs_data + 10), (void *)a, sizeof(char));
            // serial_trace("%s", a);
        }
        memset(in_data, 0, 32);
...
uint8_t *
ehci_block_read(void *this, uint64_t offset, size_t _count)
{
    // no used in initrd
    // return (uint8_t *)((uintptr_t)(((block_device_operations_t *)this)->ctx) + offset);
    return 0;
}
...
ehci_block_impl()
{
    block_device_operations_t *ops =
        (block_device_operations_t *)phys_base_alloc(1 + sizeof(block_device_operations_t) / 4096);
    // ops->read = ehci_block_read;
    // ops->ctx  = (void *)vfs_data;
    return ops;
}

struct vfs_open_response *
ehci_vfs_open(block_device_operations_t *block_op, const char *path, int _inode)
{
    if (strncmp(path, "raw", 4) == 0)
    {
        struct vfs_open_response *res = (struct vfs_open_response *)phys_base_alloc(
            1 + sizeof(struct vfs_open_response) / 4096);

        // serial_trace("%x %s", in_data[i], a);
        // }
        res->addr = (uintptr_t)vfs_data;
        res->size = 8;
        return res;
    }
    else if (strncmp(path, "asci", 4) == 0)
    {
        struct vfs_open_response *res = (struct vfs_open_response *)phys_base_alloc(
            1 + sizeof(struct vfs_open_response) / 4096);

        res->addr = (uintptr_t)(vfs_data + 10);
        res->size = 4;
        return res;
    }

    // memset(vfs_data, 0, 4096);
    // struct vfs_open_response *res = (struct vfs_open_response *)ehci_alloc(
    //     sizeof(struct vfs_open_response));
    return 0;
}

int
ehci_vfs_mount(vfs_inode_t *node)
{
    serial_trace("initrd mount\n");

    uint64_t current_offset = node->offset;

    // TarHeader *header = (TarHeader *)node->block->ops->read(node->block->ops,
    // current_offset, 0);

    // while (strncmp(header->ustar, "ustar", 5) == 0) {
    //     int size = initrd_oct2bin(header->size, 11);
    //     // if (strncmp(header->filename, entry->name, sizeof(entry->name)) ==
    //     0) { serial_trace("ON MOUNT: found path %s\n", header->filename);
...
boolean_t
ehci_is_transaction_is_running()
{
