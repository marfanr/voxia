...
void txinit() {
  ...
  // khmalloc returns a virtual address that is identical to it physical
  // mapped address. In your case you should handle virtual and physical
  // addresses as the addresses passed to the NIC should be physical ones
...
void eth_e1000_init() {
  for (int i = 0; i < 32; i++) {
    ...
    if (subclass == 0x0 && class == 0x2) {
      serial_send_string("\nFound E1000 Ethernet card device\n");
...
void e1000_irq() {
  ...
  if (status & 0x80) {
    ...
    while ((rx_desc[rx_cur]->status & 0x1)) {
      ...
      phys_base_free((void *)VIRT2PHYS((uint64_t)data), 1 + (len + 16) / 4096);
  ...
  if (status & (1 << 6)) {
    ...
    // Here you should inject the received packet into your network
    // stack
    serial_trace("ETHERNET Received packet of length %d\n", len);
  }
}
...
void send_ping(uint8_t *ip) {
  ...
  // ip_packet->length = switch_endian16(sizeof(ip_header_t) +
  // sizeof(icmp_t)); ip_packet->id = 0; ip_packet->flags = 0; ip_packet->ttl
  // = 64; ip_packet->protocol = 1; ip_packet->checksum = 0;
  // memcopy(ip_packet->src, ip, 4);
  // memcopy(ip_packet->dest, ip, 4);
  // ip_packet->checksum =
  //     switch_endian16(checksum((uint16_t *)ip_packet,
  //     sizeof(ip_header_t)));
  ...
  sendPacket((void *)VIRT2PHYS(data), sizeof(ethernet_header_t) + sizeof(ip_header_t) +
