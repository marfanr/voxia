...
uint8_t pci_readb(uint8_t bus, uint8_t device, uint8_t func, uint8_t offset) {
  ...
  address = (uint32_t)(((uint32_t)bus << 16) | ((uint32_t)device << 11) |
                       ...
                       ((uint32_t)0x80000000));
  outl(PCI_COMMAND, address);
  tmp = (uint8_t)((inb(PCI_DATA) >> ((offset & 3) * 8)) & 0xFF);
  return tmp;
}
...
static void pci_gather_info(size_t bus, size_t device, size_t func) {
  ...
  pci->capability_ptr = pci_readw(bus, device, func, 52) & 0xFF;
  ...
  case 0x00:
    ...
    case 0x40:
      ...
      break;
    case 0xFF:
      serial_trace("Device does not fit in any defined class\n");
      break;
    default:
  ...
  case 0x1AF4: {
    ...
    case 0x1050:
      serial_trace("found virtio VGA device\n");
      /* pci->service.init = virtio_gpu_init; */
      break;
    }
    break;
  }
  ...
  for (size_t device = 0; device < 32; device++) {
    ...
    if (header_type & 0x80) {
      for (size_t func = 0; func < 8; func++) {
        uint16_t vendor_id = pci_readw(bus, device, func, 0);
        if (vendor_id == 0xFFFF)
          continue;
...
void pci_scan() {
  ...
  size_t pci_allocator_estimate_size =
      (32 * 8 * sizeof(struct IoForgePCI)) + sizeof(struct buddy_allocator);
  pci_allocator =
      buddy_allocator_install((void *)((uintptr_t)phys_base_alloc(
