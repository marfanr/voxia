...
uint8_t
pci_readb(uint8_t bus, uint8_t device, uint8_t func, uint8_t offset)
{
    uint32_t address = (0x80000000U | (bus << 16) | (device << 11) | (func << 8) | (offset & 0xFC));
    outl(PCI_COMMAND, address);
...
pci_gather_info(size_t bus, size_t device, size_t func)
{
    ...
    if ((pci->status & (1 << 4)))
    {
        ...
        {
            uint8_t cap_id = pci_readb(bus, device, func, cap_ptr); // ID capability
            uint8_t next_ptr =
    ...
    uint32_t expansion_rom_base_address = pci_readw(bus, device, func, 48) & 0xFFFF;
    ...
    for (size_t device = 0; device < 32; device++)
    {
        ...
        if (header_type & 0x80)
        {
            for (size_t func = 0; func < 8; func++)
            {
                ...
                LOG_INFO("PCI", "PCI device found at %d:%d:%d", bus, device, func);
