...
static inline uint32_t pci_read32(uint64_t base, uint8_t bus, uint8_t dev,
                                  uint8_t func, uint16_t offset) {
	uint64_t addr = base + ((uint64_t)bus << 20) + ((uint64_t)dev << 15) +
	                ((uint64_t)func << 12) + offset;
	return *(volatile uint32_t*)addr;
}

static boolean_t mcfg_existed = false;

boolean_t vxIsMCFGExisted() { return mcfg_existed; }
...
void mcfg_parse(uintptr_t addr) {
	...
	size_t device_count =
	    (sdt->Length - header_size) / sizeof(MCFG_configuration_space_t);
	...
	for (size_t i = 0; i < device_count; i++) {
		...
		LOG2_INFO("PCIE CONF", "start : %d  end : %d", start, end);
		...
		LOG_INFO("PCIE CONF", "found vaddr : 0x%x (%d KB)", vaddr,
		         mapping_pages * 4);
		...
		LOG_INFO("PCIE CONF", "base addr : 0x%x -> 0x%x", cs->base_addr,
		         vaddr);
		...
		for (uint16_t bus = start; bus <= end; bus++) {
			for (uint8_t dev = 0; dev < 32; dev++) {
				 ...
				 * device */
				uint32_t id0 =
				    pci_read32(vaddr, bus, dev, 0, 0x00);
				uint16_t vendor = id0 & 0xFFFF;
				if (vendor == 0xFFFF)
					continue;

				uint8_t header =
				    (pci_read32(vaddr, bus, dev, 0, 0x0C) >>
				     16) &
				    0xFF;
				...
				for (uint8_t func = 0; func < func_count;
					...
					uint32_t id =
					    (func == 0)
					        ? id0
					        : pci_read32(vaddr, bus, dev,
					                     func, 0x00);
					if ((id & 0xFFFF) == 0xFFFF)
						continue;

					uint32_t reg08 = pci_read32(
