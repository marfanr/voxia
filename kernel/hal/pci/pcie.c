...
static uint32_t ecam_read32(uintptr_t base, uint8_t bus, uint8_t dev, uint8_t func, uint16_t offset) {
	uintptr_t addr = base + ((uintptr_t)bus << 20) + ((uintptr_t)dev << 15) + ((uintptr_t)func << 12) + offset;
...
extern void register_segment(uint16_t seg_id, uint8_t start, uint8_t end, uintptr_t vbase, pci_access_ops_t* ops, PCI_SEGMENT_TYPE type);
...
void mcfg_parse(uintptr_t addr) {
	...
	size_t device_count = (sdt->Length - header_size) / sizeof(MCFG_configuration_space_t);
	...
	for (size_t i = 0; i < device_count; i++) {
		...
		size_t mapping_pages = (mapping_size + PAGE_SIZE_4KB - 1) / PAGE_SIZE_4KB;

		uintptr_t vaddr = vma_lookup_free_vaddr(get_kernel_vmm_page(), VMA_REGION_C, mapping_pages);
		if (!vaddr) {
			LOG_INFO("pcie", "vaddr is 0");
		}
		LOG_INFO("PCIE CONF", "found vaddr : 0x%lx (%d KB)", vaddr, mapping_pages * 4);

		paging_multiple_mmap(paging_get_highest_page_map(), vaddr, cs->base_addr, mapping_pages, 0b10011);
		paging_reload(paging_get_highest_page_map());

		LOG_INFO("PCIE CONF", "base addr : 0x%lx -> 0x%lx", cs->base_addr, vaddr);

		uintptr_t adjusted_base = vaddr - ((uintptr_t)start << 20);

		register_segment(cs->pci_segment_group, cs->start_pci_bus, cs->end_pci_bus, adjusted_base, &ecam_ops, PCI_SEGMENT_PCIE);
	}
}