...
intel_hda_init (void)
{
    ...
    hda_corr = (volatile uint32_t *)((((uint32_t)hda_corr + 0x80 - 1) / 0x80)
                                     * 0x80);
...
get_connection_list (int codec, int node, uint32_t connection_number)
{
    ...
    if ((connection_lst_cap & 0x80) == 0x00)
        { // short form
            return (send_verb_pio (codec, node, 0xF02,
                                   ((connection_number / 4) * 4))
...
set_gain_node (int codec, int node, int type, uint16_t cap, uint16_t gain)
{
    ...
    if (gain == 0 && (cap & 0x80000000))
        ...
        payload |= (((cap >> 8) & 0x7F) * gain
                    / 100); // konversi 0-100 ke range node
...
audio_function_group_process (int codec, int node)
{
    ...
    serial_trace ("Supported format: AC3: %d, Float32: %d, PCM: %d\n",
                  (supported_format & 0b100) != 0,
                  (supported_format & 0b010) != 0,
    ...
    for (uint8_t i = start_node; i < start_node + total_nodes; i++)
        {
            ...
            if (widget_type == 0)
            ...
            else if (widget_type == 0x4)
                {
                    ...
                    widget_type
                        = (send_verb_pio (codec, i, 0xF1C, 0x00) >> 20) & 0xF;
                    if (widget_type == 0x0)
                        {
                        ...
                        serial_trace (
                            "Widget 0x%x is a Digital Other Out widget\n", i);
                    else if (widget_type == 0x6)
                        serial_trace (
                            "Widget 0x%x is a Modem Line Side widget\n", i);
                    else if (widget_type == 0x7)
                        serial_trace (
                            "Widget 0x%x is a Modem Handset Side widget\n", i);
                        ...
                        serial_trace (
                            "Widget 0x%x is a Digital Other In widget\n", i);
            ...
            if (out_amp_present)
                {
                    ...
                    if ((send_verb_pio (codec, i, 0xF1C, 0) >> 30) != 0x1)
                        {
                            if ((send_verb_pio (codec, i, 0xF00, 0x0C) & 0x10)
                                == 0x10)
                                {
                                    serial_trace (
                                        "Codec %d, Node 0x%x connected to "
                                        "output device\n",
                                        codec, i);
                                    pin_speaker_node = i;
                                }
            ...
            uint16_t con_list
                = get_connection_list (codec, i, con_entry_number);
            while (con_list != 0)
                {
    ...
    if (pin_speaker_node != 0)
        {
            ...
            uint16_t pin_out_cap
                = send_verb_pio (codec, pin_speaker_node, 0xF00, 0x012);
    ...
    else if (pin_alt_speaker_node != 0)
        {
            ...
            send_verb_pio (codec, pin_alt_speaker_node, 0x706,
                           0x10); // Set stream/channel
            uint16_t pin_out_cap
                = send_verb_pio (codec, pin_alt_speaker_node, 0xF00, 0x012);
...
uint16_t
hda_return_sound_data_format (uint32_t sample_rate, uint32_t channels,
...
intel_hda_play (const char *file_path)
{
    ...
    uint8_t *file
        = (uint8_t *)VIRT2PHYS (phys_base_alloc ((stats.size / 4096) + 1));
    ...
    volatile uint32_t *hda_stream_output_base
        = (volatile uint32_t *)(hda_device.bar[0] + 0x80
                                + (0x20 * ((mmio_readw (0x00) >> 8) & 0xF)));
    ...
    *((volatile uint32_t *)(hda_stream_output_base + 0x03)) = 0x1C;
    ...
    struct stream_buffer *stream_list
        = (struct stream_buffer *)((uint64_t)VIRT2PHYS (
            phys_base_alloc (1 + (sizeof (struct stream_buffer) * 2))));
    stream_list
        = (struct stream_buffer *)(((uint32_t)stream_list + 0x7F) & ~0x7F);
    ...
    mmio_writel (0x80 + pos * 0x20 + 0x1C,
                 (uint32_t)((uintptr_t)stream_list >> 32));
    ...
    mmio_writel (0x80 + pos * 0x20 + 0x0,
                 mmio_readl (0x80 + pos * 0x20 + 0x0) | 0x2);
