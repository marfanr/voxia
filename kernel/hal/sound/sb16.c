...
sb16_play (const char *name)
{
    ...
    vfs_fstat (fd, &stats);
    uint8_t *buf_
        = (uint8_t *)VIRT2PHYS (phys_base_alloc ((1 + stats.size) / 4096));
...
sb16_init ()
{
            ...
            if (pci_devices[i].class == 0x4 && pci_devices[i].subclass == 0x1)
                {
                    serial_trace (
                        ...
                        pci_devices[i].bar[0], pci_devices[i].bar[1]);
    ...
    buf = (bufer_list *)VIRT2PHYS (
        phys_base_alloc (1 + (sizeof (bufer_list) * 32) / 4096));
