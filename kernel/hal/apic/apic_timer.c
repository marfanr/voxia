...
void
timer_handle(interrupt_stack_frame_t *_)
{
...
static void
calibrate_tsmc_backend(interrupt_stack_frame_t *_)
{
    tsmc_calibrated = 1;
}

boolean_t
vxAPICIsTSMCSupported()
{
...
vxAPICTimerCalibrationUsingHPET()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        uint64_t hpet_ticks = us2ns(1) / tick_ns; // Konversi 1ms ke nanodetik
        ...
        while ((vxTimerCounterCount(&counter)) < 1e3)
            ;
        double elapsed_time_us = vxTimerCounterCountInNs(&counter) * 1e-3;

        // correction
        double elapsed_test = 0xFFFFFFFF - apic_read(TIMER_CURRENT_COUNT);

        double error_ratio = 1.0f / elapsed_time_us;
        double elapsed     = elapsed_test * error_ratio;

        calibrated_ticks_1ns = calibrated_ticks_1ns * i + (double)elapsed;
        calibrated_ticks_1ns /= (i + 1);
    }

    LOG_DEBUG("APIC_TIMER", "[APIC] calibrated apic timer 1ms done.");
}
...
void
vxAPICCreateTimer(uint32_t type, double freq_us, uint8_t vector)
{
    apic_write(TIMER_DIVIDE_CONFIG, 0b1011);
    ...
    {
        LOG_ERROR("APIC_TIMER", "Invalid interrupt vector: 0x%x", vector);
        return;
    }
    ...
    if (type == APIC_TIMER_ONE_SHOT)
    {
        ...
        {
            LOG_WARN("APIC_TIMER", "Initial count truncated to 32-bit");
            initial_count = 0xFFFFFFFF;
        }
    }
    else // APIC_TIMER_PERIODIC
    {
        // Untuk periodic: initial count juga berfungsi sebagai reload value
        initial_count = calibrated_ticks_1ns * freq_us;

        if (initial_count > 0xFFFFFFFF)
        {
            LOG_WARN("APIC_TIMER", "Period too long, truncated to 32-bit");
            initial_count = 0xFFFFFFFF;
        }
    }
...
void
vxAPICCreateDeadlineTimer(const uint8_t vector, const double freq_us)
{
