...
INIT(gdt)
{
    ...
    gdt_entries[2] = gdt_make_entry(0, 0xFFFF, 0x92, 0x00); // 0x10 16 bit data segment
    // 32 bit memiliki base dan limit yang sama dengan 16 bit
    // namun memiliki flag yang berbeda yaitu pada granularitas
    //   32 bit
    gdt_entries[3] = gdt_make_entry(0, 0xFFFF, 0x9A, 0xCF); // 0x18 32 bit code segment
    gdt_entries[4] = gdt_make_entry(0, 0xFFFF, 0x92, 0xCF); // 0x20 32 bit data segment
