...
void
usleep_backend(interrupt_stack_frame_t *_)
{
    ...
    if (core->core_id > 1)
        LOG_DEBUG("TIMER", "usleep backend triggerred on core %d", core->core_id);
}

void
usleep(const double time_ns)
{
...
INIT(Timer)
{
    LOG_INFO("TIMER", "Initializing HPET timer");
...
void
vxTimerCounterInit(time_counter_t *counter)
{
    counter->current = vxHPETGetMainCount();
}

uint64_t
vxTimerCounterCount(time_counter_t *counter)
{
    return vxHPETGetMainCount() - counter->current;
}

double
vxTimerCounterCountInMs(time_counter_t *counter)
{
    return ns2ms((double)(vxHPETGetMainCount() - counter->current) * vxHPETMinTickNs());
}

double
vxTimerCounterCountInNs(time_counter_t *counter)
{
