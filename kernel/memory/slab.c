...
slab_alloc(struct slab_cache *cache)
{
    ...
    if (slab == NULL)
    {
        ...
        if (slab == NULL)
        {
            ...
            slab->first_obj = (void *)((uintptr_t)slab + sizeof(struct slab));
            size_t size     = cache->obj_size;
            if (cache->alignment > 0 && cache->alignment > sizeof(void *))
            {
                size += cache->alignment - sizeof(void *);
            }
            slab->total_objects =
                (cache->slab_size - sizeof(struct slab)) / (size + sizeof(void *));
            ...
            {
                *(void **)obj = (void *)(obj + cache->obj_size + sizeof(void *));
...
void
slab_free(struct slab_cache *cache, void *obj)
{
    ...
    while (slab)
    {
        if ((uintptr_t)obj >= (uintptr_t)slab->first_obj &&
            (uintptr_t)obj < (uintptr_t)slab->first_obj +
                                 slab->total_objects * (cache->obj_size + sizeof(void *)))
        {
            break;
        }
        prev = slab;
