...
INIT(phys_base_allocator)
{
    ...
    for (uint64_t i = 0; i < ctx->memory.memory_entries; i++)
    {
        ...
        if (entry->base > 0)
        {
            ...
            else if (entry->base < smalest_base2 && entry->base != smalest_base1)
            {
    ...
    LOG_INFO("MEMORY", "metadata  count %d with size %d kb", metadata_count, metadata_size / 1024);

    for (uint64_t i = 0; i < ctx->memory.memory_entries; i++)
    {
        memory_entry_t *entry = &ctx->memory.memory_map[i];

        if (entry->length >= metadata_size && entry->type == STIVALE2_MMAP_USABLE)
        {
            LOG_INFO("MEMORY", "found suitale place for metadata 0x%x length %d kb", entry->base,
                     entry->length / 1024);
    ...
    for (uint64_t i = 0; i < higher_base_length_ / BLOCK_SIZE; i++)
    {
        ...
        {
            LOG_INFO("MEMORY", "found smallest entry at index %d", i);
            break;
        }
    }

    // mark smallest entry sebagai reserved atau digunakan
    LOG_INFO("MEMORY", "usable memory size : %d mb", usable_total / 1024 / 1024);
    for (uint64_t i = 0; i < ctx->memory.memory_entries; i++)
    {
        memory_entry_t *entry = &ctx->memory.memory_map[i];
...
dma_alloc(uint64_t block)
{
    ...
    for (uint64_t i = 0; i < dma_bitmap_count * 8; i++)
    {
        ...
        if (!used)
        {
            ...
            {
                for (uint64_t j = start; j < start + block; j++)
                    dma_bitmap_base_[j / 8] |= (1 << (j % 8));

                // serial_trace("allocated %llu blocks starting at %llu\n", block, start);
                return (void *)(smallest_free_entry_base + start * BLOCK_SIZE);
            }
        }
        else
        {
