...
extern size_t __fast_phys_base_find_free_block__(uint64_t* bitmap,
                                                 size_t num_words);
...
INIT(phys_base_allocator) {
	...
	// metadata (round up to ensure all blocks are covered)
	metadata_count = (higher_base_length_ / BLOCK_SIZE + 7) / 8;
	...
	for (uint64_t i = 0; i < ctx->memory.memory_entries; i++) {
		...
		if (entry->length >= metadata_size &&
		    entry->type == ENTRY_MMAP_USABLE) {
			LOG_INFO("MEMORY",
			         "found suitale place for metadata 0x%x length "
			         "%d kb",
			         entry->base + ctx->hhdm_offset,
			         entry->length / 1024);

			bitmap_base_ =
			    (uint8_t*)(ctx->hhdm_offset + entry->base);
...
void* phys_base_alloc(uint64_t block) {
	...
	while (i < total_blocks) {
		...
		uint64_t word = *(uint64_t*)(void*)&bitmap_base_[word_idx * 8];
...
void pmm_log_usage() {
	...
	LOG2_INFO("PMM", "used memory : %d mb / %d mb (%d mb free)\n",
