...
INIT(vma) {
	LOG_DEBUG("VMA", "zone_a active %x, unused %x",
	          (void*)vma_tree_zone_a.active, (void*)vma_tree_zone_a.unused);
	LOG_DEBUG("VMA", "zone_b active %x, unused %x",
	          (void*)vma_tree_zone_b.active, (void*)vma_tree_zone_b.unused);
	LOG_DEBUG("VMA", "zone_c active %x, unused %x",
	          (void*)vma_tree_zone_c.active, (void*)vma_tree_zone_c.unused);
...
virtual_memory_t* vma_find(uintptr_t virt_addr) {
	struct rbt_node* n =
	    rbt_search_node(virtual_memory_tree_root, virt_addr, VMA_RBT_NIL);
	return n->data;
}
...
void vma_rbt_debug(rbt_node* root) { vma_rbt_debug_node(root, 0); }
