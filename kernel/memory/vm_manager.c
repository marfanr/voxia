...
static struct virtual_memory_tree vma_tree_zone_process = {0};
...
INIT(vma) {
	...
	vma_tree_zone_process.active = 0;
	vma_tree_zone_process.unused = 0;
}
...
void vma_tree_add(mem_vma_region region, uintptr_t start_address,
	...
	case VMA_REGION_PROCESS: {
		node->next = vma_tree_zone_process.active;
		vma_tree_zone_process.active = node;
		break;
	}
	}
}
...
uintptr_t vma_lookup_free_vaddr(mem_vma_region region, size_t size) {
	...
	case VMA_REGION_PROCESS:
		curr = vma_tree_zone_process.active;
	default:
		break;
	}
