...
INIT(KernelReader)
{
    ...
    uintptr_t addr = vma_lookup_free_vaddr(VMA_REGION_A, ctx->kernel_raw_size / BLOCK_SIZE);
    vxMultipleMmap(paging_get_highest_page_map(), addr, (uintptr_t)ctx->kernel_raw_addr,
    ...
    for (uint64_t i = 0; i < sym_count; i++)
    {
        ...
        symbols_register(&ksymbols, (const char *)(strtab + symtab[i].st_name),
