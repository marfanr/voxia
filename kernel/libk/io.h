...
static inline void
mmio_outw(uint32_t addr, uint16_t value)
{
    asm volatile("outw %0, %1" : : "a"(value), "Nd"(addr));
}

static inline uint16_t
mmio_inw(uint32_t addr)
{
    uint16_t ret;
...
static inline void
mmio_outl(uintptr_t addr, uint32_t value)
{
    *((volatile uint32_t *)addr) = value;
}

static inline uint32_t
mmio_inl(uintptr_t addr)
{
    return *((volatile uint32_t *)addr);
}

static inline void
mmio_outll(uintptr_t addr, uint64_t value)
{
    *((volatile uint64_t *)addr) = value;
}

static inline uint64_t
mmio_inll(uintptr_t addr)
{
