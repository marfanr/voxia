    ...
    for (int i = 0; dyn[i].d_tag != DT_NULL; i++)
    {
        switch (dyn[i].d_tag)
        {
            ...
            case DT_PLTRELSZ:
                ...
                LOG_INFO("ELF", "pltrel size : %x", map->pltrelsz);
                break;
            ...
            case DT_RELA:
                LOG_INFO("ELF", "found rela at : 0x%x", data + dyn[i].d_un.d_ptr);
                map->rel = (Elf64_Rela *)(data + dyn[i].d_un.d_ptr);
                break;

            case DT_RELASZ:
                map->relasz = dyn[i].d_un.d_val;
                LOG_INFO("ELF", "relasz : %x", map->relasz);
                break;

            case DT_RELAENT:
                map->relaent = dyn[i].d_un.d_val;
                LOG_INFO("ELF", "relaent : %x", map->relaent);
                break;
...
void
elf_mmap_got(elf_section_map *map, uintptr_t base)
{
    if (map->gotplt)
    {
        ...
        LOG_INFO("VOXMO", "gotplt found at 0x%x (0x%x), size %d", base + alligned_start,
                 offset_aligned, total_aligned);

        uintptr_t phys_addr = (uintptr_t)vxPhysBaseAlloc(total_aligned);
        vxMultipleMmap(paging_get_highest_page_map(), base + alligned_start, phys_addr,
                       total_aligned, 0b111);
...
uintptr_t
elf_get_entry(uint8_t *data, uintptr_t base)
{
    Elf64_Ehdr *ehdr = (Elf64_Ehdr *)data;
...
size_t
elf_load(uint8_t *data, uintptr_t base)
{
    Elf64_Ehdr *ehdr = (Elf64_Ehdr *)data;
    ...
    for (uint64_t i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        vxMultipleMmap(paging_get_highest_page_map(), base + aligned_vaddr, (uintptr_t)a, sz,
                       0b111);
        paging_reload(paging_get_highest_page_map());
        memcopy((void *)(base + p->p_vaddr), (void *)((uint8_t *)data + p->p_offset), p->p_filesz);
    }
...
static uintptr_t
elf_resolve_external_symbol(symbols_ptr_vector_t *external_syms, const char *name)
{
    uintptr_t result = 0;
    if (external_syms)
    {
        for (size_t j = 0; j < external_syms->size; j++)
        {
            ...
            {
                // LOG_DEBUG("ELF", "checking external symbol %s", item->data[k].name);
                if (strncmp(item->data[k].name, name, strlen(name)) == 0)
                {
...
void
elf_relocate_rel(Elf64_Rela *rel, uintptr_t base, uint64_t rela_count, uint8_t *strtab,
                 Elf64_Sym *symbols, GnuHashHeader *gnu_hash, symbols_ptr_vector_t *external_syms)
{
    for (uint64_t i = 0; i < rela_count; i++)
    {
        ...
        // LOG_INFO("ELF", "[offset 0x%x] [%d] symbol %s, value 0x%x, size %d", rel[i].r_offset, i,
        //  name, symbol->st_value, symbol->st_size);

        Elf64_Sym *lookup_sym = elf_gnu_lookup(name, gnu_hash, symbols, (const char *)strtab);
        if (lookup_sym)
        {
        ...
        switch (type)
        {
            ...
            {
                uint64_t *relloc_addr = (uint64_t *)(base + rel[i].r_offset);
            ...
            {
                uint64_t *relloc_addr = (uint64_t *)(base + rel[i].r_offset);
                *relloc_addr          = base + rel[i].r_addend + symbol->st_value;
                break;
            }

            case R_X86_64_COPY:
                memcopy((void *)(base + rel[i].r_offset),
                        (void *)(base + symbol->st_value), // atau alamat eksternal
                        symbol->st_size);
                break;
            ...
            {
                uintptr_t value = base + symbol->st_value + rel[i].r_addend;
                if (!symbol->st_value)
                {
                    LOG_DEBUG("ELF", "resolving external symbol %s", name);
                    value = elf_resolve_external_symbol(external_syms, name);
                }
                if (!value)
                {
                ...
                *(uint64_t *)(base + rel[i].r_offset) = value;
                LOG_DEBUG("ELF", "update gotplt at 0x%x (0x%x + %x) to 0x%x",
                          base + rel[i].r_offset, base, rel[i].r_offset, value);
            ...
            default:
            {
                *(uint64_t *)(base + rel[i].r_offset) = base + symbol->st_value;
                LOG_DEBUG("ELF", "update gotplt at 0x%x (0x%x + x) to 0x%x", base + rel[i].r_offset,
                          base, rel[i].r_offset, base + symbol->st_value);
                break;
            }
        }
    }
...
void
elf_relocate_dyn(elf_dynamic_map *map, uintptr_t base, GnuHashHeader *gnu_hash,
                 symbols_ptr_vector_t *external_syms)
{
    ...
    if (map->rel)
    {
        LOG_INFO("VOXMO", "rela found at 0x%x", map->rel);
        uint64_t rela_count = map->relasz / map->relaent;
        LOG_INFO("VOXMO", "rela count %d", rela_count);
        elf_relocate_rel(map->rel, base, rela_count, map->strtab, map->symbols, gnu_hash,
                         external_syms);
    }

    if (map->jmprel && map->pltrelsz && map->relaent)
    {
        LOG_INFO("VOXMO", "jmprel found at 0x%x", map->jmprel);
        uint64_t rela_count = map->pltrelsz / map->relaent;
        LOG_INFO("VOXMO", "rela count %d", rela_count);
        elf_relocate_rel(map->jmprel, base, rela_count, map->strtab, map->symbols, gnu_hash,
                         external_syms);
    }
}
...
uintptr_t
elf_count_load_size(uint8_t *data)
{
    Elf64_Ehdr *ehdr    = (Elf64_Ehdr *)data;
    Elf64_Phdr *phdr    = (Elf64_Phdr *)(data + ehdr->e_phoff);
    size_t      max_end = 0;
...
int
elf_gnu_maybe_present(GnuHashHeader *gh, uint32_t hash)
{
    // Setiap elemen bloom adalah 64-bit (Elf64_Addr)
    uint64_t word = gh->bloom[(hash / 64) % gh->bloom_size];
    uint64_t mask = (1ULL << (hash % 64)) | (1ULL << ((hash >> gh->bloom_shift) % 64));
    return (word & mask) == mask;
}

Elf64_Sym *
elf_gnu_lookup(const char *name, GnuHashHeader *gh, Elf64_Sym *symtab, const char *strtab)
{
...
void
elf_gnu_hash_parse(GnuHashHeader *gnu_hash, Elf64_Shdr *gnu_hash_sym, uint8_t *data)
{
    if (gnu_hash_sym)
    {
        ...
        // serial_trace(".gnu.hash info:\n");
        // serial_trace(" nbuckets    = %d\n", gnu_hash->nbuckets);
        // serial_trace(" symoffset   = %d\n", gnu_hash->symoffset);
        // serial_trace(" bloom_size  = %d\n", gnu_hash->bloom_size);
        // serial_trace(" bloom_shift = %d\n", gnu_hash->bloom_shift);

        // for (uint32_t i = 0; i < gnu_hash->bloom_size; i++)
        //     serial_trace(" bloom[%d] = 0x%x\n", i, gnu_hash->bloom[i]);

        // for (uint32_t i = 0; i < gnu_hash->nbuckets; i++)
        //     serial_trace(" bucket[%d] = %d\n", i, gnu_hash->buckets[i]);
    }
}

// symbol
void
elf_get_symbol(const char *sym_name, uintptr_t base, elf_section_map *map, uint8_t *data,
               symbols_ptr_vector_t *syms, boolean_t skip_empty_val)
{
    ...
    Elf64_Sym  *symtab = (Elf64_Sym *)((uint64_t)data + map->symtab->sh_offset);
    ...
    for (uint64_t i = 0; i < sym_count; i++)
    {
        ...
        const char *name = (const char *)(strtab_data + symtab[i].st_name);
        // LOG_INFO("ELF", "[%d] name=%s, value=0x%x size=%d", i, name, base + symtab[i].st_value,
        //          symtab[i].st_size);
...
void
elf_call_init_array(elf_section_map *map, uintptr_t base)
{
    if (map->init_aray)
    {
        ...
        LOG_INFO("VOXMO", "lib init array count %d", count);
        for (size_t i = 0; i < count; i++)
        {
            if (arr[i])
            {
                LOG_DEBUG("VOXMO", "load .init array %x", arr[i]);
...
uintptr_t
elf_find_symbol(const char *name, GnuHashHeader *gnuhash, uintptr_t base, elf_section_map *map,
                uint8_t *data)
{
    const char *strtab_data = (const char *)((uint64_t)data + map->strtab->sh_offset);
    Elf64_Sym  *symtab      = (Elf64_Sym *)((uint64_t)data + map->symtab->sh_offset);
    ...
    if (sym)
    {
        if (sym->st_value)
        {
            LOG_INFO("ELF", "sym found %s by gnu hash 0x%x", strtab_data + sym->st_name,
                     base + sym->st_value);
    ...
    {
        if (strncmp(strtab_data + symtab[i].st_name, name, strlen(name)) == 0)
        {
            LOG_INFO("ELF", "sym found %s 0x%x", strtab_data + symtab[i].st_name,
                     symtab[i].st_value);
