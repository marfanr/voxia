...
void elf_mmap_got(elf_section_map* map, uintptr_t base) {
	if (map->gotplt) {
		...
		uintptr_t phys_addr =
			(uintptr_t) vxPhysBaseAlloc(total_aligned);
	...
	if (external_syms) {
		for (size_t j = 0; j < external_syms->size; j++) {
			...
			for (size_t k = 0; k < item->size; k++) {
				// LOG2_DEBUG("ELF", "checking external symbol
				// %s", item->data[k].name);
				// if (strncmp(item->data[k].name, name,
				// 	    strlen(item->data[k].name))
				//     == 0) {
...
void elf_relocate_rel(Elf64_Rela* rel, uintptr_t base, uint64_t rela_count,
	...
	for (uint64_t i = 0; i < rela_count; i++) {
		...
		Elf64_Sym* lookup_sym = elf_gnu_lookup(name, gnu_hash, symbols,
						       (const char*) strtab);
		if (lookup_sym) {
			// LOG2_DEBUG("ELF", "found lookup at gnu symbl %s",
			// name);
			symbol = lookup_sym;
		}
		...
		case R_X86_64_JUMP_SLOT: {
			...
			if (!value) {
				LOG2_WARN("ELF", "symbol %s not found", name);
				break;
			}
		...
		default: {
			*(uint64_t*) (base + rel[i].r_offset) =
			...
			LOG2_DEBUG("ELF",
				   ...
				   base + rel[i].r_offset, base,
				   rel[i].r_offset, base + symbol->st_value);
			break;
		}
		}
	}
...
uintptr_t elf_find_symbol(const char* name, GnuHashHeader* gnuhash,
	...
	for (size_t i = 0; i < sym_count; i++) {
		if (strncmp(strtab_data + symtab[i].st_name, name, strlen(name))
		    == 0) {
