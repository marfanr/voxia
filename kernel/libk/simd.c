...
INIT(SIMD)
{
    ...
    if (!has_sse)
    {
        LOG_WARN("SIMD", "SSE not supported, SIMD disabled");
        simd_has_avx = false;
        return;
    }
    ...
    if (has_xsave && has_avx)
    {
        ...
        LOG_INFO("SIMD", "AVX OSX initialized via XCR0");
        simd_has_avx = true;
    }
    else if (has_xsave && !has_avx)
    {
        uint64_t xcr0 = 0b11; // x87 + SSE only
        __asm__ volatile("xsetbv" : : "a"((uint32_t)xcr0), "d"((uint32_t)(xcr0 >> 32)), "c"(0));
        LOG_INFO("SIMD", "SSE initialized via XCR0 (no AVX)");
    }
    else
    {
        LOG_WARN("SIMD", "XSAVE/AVX not supported, fallback to SSE only");
    }

    // LOG_INFO("SIMD", "SIMD (SSE/AVX) enabled successfully");
}
