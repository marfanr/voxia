...
vfs_install ()
{
    NIL = (struct vfs_rb_node *)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (struct vfs_rb_node) / 4096));
    NIL->color = RBT_BLACK;
    NIL->inode = (vfs_inode_t *)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (vfs_inode_t) / 4096));
    NIL->inode->id = 0;
    NIL->inode->entry = (struct vfs_entry *)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (struct vfs_entry) / 4096));
    NIL->left = NIL->right = NIL->parent = NIL;

    g__vfs_ = (vfs_t *)VIRT2PHYS (phys_base_alloc (1 + sizeof (vfs_t) / 4096));
    g__vfs_->inode_root = NIL;
    g__vfs_->entry_root = 0;
    g__vfs_->mount_list = 0;
    g__vfs_->fs_list = (struct vfs_fs **)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (struct vfs_fs *) * VFS_MAX_FS / 4096));
    g__vfs_->path_cache
        = (struct vfs_path_cache **)VIRT2PHYS (phys_base_alloc (
            1 + sizeof (struct vfs_path_cache *) * VFS_MAX_PATH_CACHE / 4096));
    KDEBUG (DEBUG_LEVEL_INFO, "[OK] VFS installed");
}

void
rbt_rotate_left (struct vfs_rb_node **root, struct vfs_rb_node *x)
{
...
void
rbt_rotate_right (struct vfs_rb_node **root, struct vfs_rb_node *y)
{
...
void
rbt_fix_insert (struct vfs_rb_node **root, struct vfs_rb_node *y)
{
...
int
rbt_insert_node (struct vfs_rb_node **root, vfs_inode_t *inode)
{
    struct vfs_rb_node *z = (struct vfs_rb_node *)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (struct vfs_rb_node) / 4096));
...
void
rbt_debug (struct vfs_rb_node *root)
{
    if (root != NIL)
        {
            rbt_debug (root->left);

            serial_trace ("[DEBUG] inode id : %d\n", root->inode->id);
...
explode_path (const char *path)
{
    ...
    char **result = (char **)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (char *) * strlen (path) / 4096));
    memset (result, 0, sizeof (char *) * strlen (path));

    int i = 0, j = 0;
    char *buffer = (char *)VIRT2PHYS (phys_base_alloc (1 + 256 / 4096));
    ...
    while (*path)
        {
            if (*path == '/')
                {
                    buffer[i] = 0;
                    result[j] = (char *)VIRT2PHYS (
                        phys_base_alloc (1 + sizeof (buffer) / 4096));
    ...
    if (i > 0)
        {
            buffer[i] = 0;
            result[j] = (char *)VIRT2PHYS (
                phys_base_alloc (1 + strlen (buffer) / 4096));
            strcpy (result[j], buffer);
            // serial_trace ("result[%d] : %s\n", j, result[j]);
            j++;
        }
    phys_base_free ((uint64_t)buffer, 1 + 256 / 4096);
    return result;
}

int
calc_path_depth (const char *path)
{
    int depth = 0;
    while (*path)
        {
...
#define ALLOC_INODE                                                           \
    (vfs_inode_t *)VIRT2PHYS (                                                \
        phys_base_alloc (1 + sizeof (vfs_inode_t) / 4096))
#define ALLOC_ENTRY                                                           \
    (struct vfs_entry *)VIRT2PHYS (                                           \
        phys_base_alloc (1 + sizeof (struct vfs_entry) / 4096))
...
void
save_mount (vfs_inode_t *node, struct vfs_fs *fs, struct block_device *block)
{
    struct vfs_mount *mount = (struct vfs_mount *)VIRT2PHYS (
        phys_base_alloc (1 + sizeof (struct vfs_mount) / 4096));
    mount->inode = node;
    mount->fs = fs;
    mount->block = block;
    mount->next = 0;
...
void
vfs_caching_path (const char *path, vfs_inode_t *inode,
                  struct vfs_entry *entry)
{
    int index = hash (path, VFS_MAX_PATH_CACHE);
    struct vfs_path_cache_item *cache_item
        = (struct vfs_path_cache_item *)VIRT2PHYS (
            phys_base_alloc (1 + sizeof (struct vfs_path_cache_item) / 4096));
    ...
    if (cache == 0)
        cache = (struct vfs_path_cache *)VIRT2PHYS (
            phys_base_alloc (1 + sizeof (struct vfs_path_cache) / 4096));
...
void
add_new_entry (struct vfs_entry *entry, struct vfs_entry *parent)
{
    serial_trace ("add new entry : %s parent ; 0x%x\n", entry->name, parent);
    if (parent->child_count == 0)
        {
            parent->child = (struct vfs_entry **)VIRT2PHYS (phys_base_alloc (
                1 + VFS_ENTRY_PRE_ALLOC * sizeof (struct vfs_entry *) / 4096));
        }
    else if (parent->child_count % VFS_ENTRY_PRE_ALLOC == 0)
        {
            struct vfs_entry **temp = (struct vfs_entry **)VIRT2PHYS (
                phys_base_alloc (1
                                 + (parent->child_count + VFS_ENTRY_PRE_ALLOC)
                                       * sizeof (struct vfs_entry *) / 4096));
...
vfs_mount (const char *path, const char *block, const char *fs)
{
    ...
    char *_path = (char *)VIRT2PHYS (phys_base_alloc (1 + 256 / 4096));
    memset (_path, 0, 4096);
    ...
    for (int i = 0; i < path_depth; i++)
        {
            ...
            if (curr_inode == 0 || curr_inode->id == 0)
                {
                    curr_inode = vfs_create_inode (_hash, _fs);
                ...
                {
                    phys_base_free ((uint64_t)exploded_path[i],
...
int
vfs_lookup (const char *path, int mount_inode)
{
    // serial_trace ("mount inode : %d\n", mount_inode);
    int path_depth = calc_path_depth (path);
    // serial_trace ("lookup path depth : %d\n", path_depth);
    char **exploded_path = explode_path (path);
    // serial_trace ("lookup : %s\n", path);
    struct vfs_rb_node *_mount_rb_inode
        = rbt_find_node (g__vfs_->inode_root, mount_inode);

    rbt_debug (g__vfs_->inode_root);
    if (_mount_rb_inode == 0)
        {
            // serial_trace ("mount not found\n");
    ...
    int mount_start = 0;
    char *_path
        = (char *)VIRT2PHYS (phys_base_alloc (1 + strlen (path) / 4096));
    ...
    for (int i = 0; i < path_depth; i++)
        {
            ...
            // serial_trace ("full path : %s  %s\n", _full_path,
            //               exploded_path[i]);

            if (strncmp (exploded_path[i], _mount_inode->entry->name,
                         strlen (exploded_path[i]))
                == 0)
                mount_start = i;

            // serial_trace ("mount start : %d\n", mount_start);

            if (i > 0 && i > mount_start && mount_start > 0)
                {
                    strcat (_path, exploded_path[i]);
                    // serial_trace (" strcat %s %s\n", _path,
                    // exploded_path[i]);
                    if (i > 0 && i < path_depth - 1)
                        strcat (_path, "/");

                    // serial_trace (" | searching in real FS :%s\n", _path);
                    struct vfs_open_response *open_response
                        = _mount_inode->fs->ops->open (mount->block->ops,
                                                       _path, curr_inode->id);

                    int id = _hash;
                    if (mount->fs->has_own_inode)
                        id = open_response->id;
                    // serial_trace ("inode id : %d hash %d\n", id, _hash);
                    ...
                    if (curr_inode == 0 || curr_inode->id == 0)
                        {
                            curr_inode
                                = vfs_create_inode (_hash, _mount_inode->fs);
                            struct vfs_entry *entry = vfs_create_entry (
                                exploded_path[i], curr_inode, curr_entry);
                            curr_inode->id = id;
                            entry->parent = curr_entry;
                            curr_inode->entry = entry;
                            curr_inode->is_directory
                                = open_response->is_directory;
...
int
get_curr_mount_inode_id_by_path (const char *path)
{
    int path_depth = calc_path_depth (path);
    char **exploded_path = explode_path (path);

    char *_path = (char *)VIRT2PHYS (phys_base_alloc (1));
    memset (_path, 0, 4096);
    ...
    phys_base_free ((uint64_t)_path, 1);
    if (!mount_found)
        return -1;
    return hash_path;
}
...
int
vfs_open (const char *path, uint8_t flags)
{
    ...
    int fd = descriptor_add (file, file_node->entry->addr, 0, _flags);
    serial_trace ("fd : %d\n", fd);
...
int
vfs_fstat (int fd, struct vfs_file_stats *buf)
{
