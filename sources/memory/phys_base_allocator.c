...
phys_base_allocator_install (struct stivale2_struct_tag_memmap *memmap)
{
    ...
    bitmap_size_ = ALIGN_UP (
        ALIGN_DOWN (higher_base_length_, 0x1000) / 0x1000 / 32, 0x1000);
    ...
    for (uint64_t i = 0; i < memmap->entries; i++)
        {
            ...
            if (entry->length >= bitmap_size_)
                {
                    bitmap_base_ = (uint32_t *)(PHYS2VIRT (entry->base));
    ...
    KDEBUG (DEBUG_LEVEL_INFO,
            "bitmap physical memory allocator hosted at: 0x%x", bitmap_base_);
    ...
    for (uint64_t i = 0; i < memmap->entries; i++)
        {
            ...
            if (!(entry->type == STIVALE2_MMAP_USABLE
                  ...
                  || entry->type == STIVALE2_MMAP_FRAMEBUFFER
...
phys_base_free (void *ptr, uint64_t size)
{
    ...
    for (uint64_t i = 0; i < size; i++)
        bitmap_base_[(index + i) / 32] &= ~(1 << ((index + i) % 32));
}
...
pmm_log_usage ()
{
    ...
    KDEBUG (DEBUG_LEVEL_INFO, "used memory : %d mb \n", used / 1024);
}